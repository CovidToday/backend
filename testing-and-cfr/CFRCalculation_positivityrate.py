    ...
    for j in range(len(state_dates)):
      ...
      if (dates[i]==state_dates[j]):
        ...
        if(len(tested_cum[i])!=0 and len(tested_cum[i-1])!=0):
          daily_tested[i]=abs(int(tested_cum[i])-int(tested_cum[i-1]))
          test_per_million[i]=abs(int(tested_cum[i])*1000000/int(population["Population"][st]))
...
#               print(sum)
  ...
  for i in range(7,len(daily_pos)):
    ...
    if (sum2!=0):
      daily_pos_rate_ma[i]=sum1*100/abs(sum2)
  if(st=="Maharashtra"):
